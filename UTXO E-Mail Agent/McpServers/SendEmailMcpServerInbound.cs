...
/// Used when agent's email provider is "inbound"
/// </summary>
public class SendEmailMcpServerInbound : ISendEmailMcpServer
{
    ...
    public SendEmailMcpServerInbound(IConfiguration config, string fromAddress, int agentId)
    {
    ...
    {
        Logger.Log($"[SendEmail Inbound] ========================================", _agentId);
        Logger.Log($"[SendEmail Inbound] Sending email to: {to}", _agentId);
        Logger.Log($"[SendEmail Inbound] Subject: {subject}", _agentId);
        Logger.Log($"[SendEmail Inbound] From: {_fromAddress}", _agentId);
        if (!string.IsNullOrEmpty(replyTo))
            Logger.Log($"[SendEmail Inbound] Reply-To: {replyTo}", _agentId);
        ...
        try
        {
            ...
            Logger.Log($"[SendEmail Inbound] Payload: {jsonPayload}", _agentId);
            ...
            {
                Logger.Log($"[SendEmail Inbound] Success: {response.StatusCode}", _agentId);
                Logger.Log($"[SendEmail Inbound] Response: {responseContent}", _agentId);
                Logger.Log($"[SendEmail Inbound] ========================================", _agentId);
                return $"Email successfully sent to {to} with subject '{subject}'";
            }
            else
            {
                Logger.LogError($"[SendEmail Inbound] Error: {response.StatusCode}", _agentId);
                Logger.LogError($"[SendEmail Inbound] Response: {responseContent}", _agentId);
                Logger.LogError($"[SendEmail Inbound] ========================================", _agentId);
                return $"ERROR: Failed to send email. Status: {response.StatusCode}, Response: {responseContent}";
            }
        }
        catch (Exception ex)
        {
            Logger.LogError($"[SendEmail Inbound] Exception: {ex.Message}", _agentId);
            Logger.LogError($"[SendEmail Inbound] ========================================", _agentId);
